...
def simple_auto_fix(code):
    ...
    # Исправляем двоеточие у функций
...
def process_message(message):
    ...
    if text == "/start" or text == "🔙 Назад":
        send_message(chat_id, "🤖 AI Code Bot\n\nПришли код - я исправлю ошибки!\n\n📤 Отправляй код частями\n🗑 Удалить последний - отменить последнюю часть\n🔧 ИСПРАВИТЬ - исправлю все ошибки", reply_markup=json.dumps(get_keyboard()))
    ...
    # НОВАЯ КНОПКА: Удалить последнюю часть
        ...
        if not history:
            ...
            if history:
                ...
                send_message(chat_id, f"🗑 Удалена последняя часть:\n```\n{last['part'][:200]}\n```\nОсталось частей: {len(history)}", parse_mode="Markdown")
            else:
    ...
    # Обработка обычного кода (не команды и не кнопки)
        ...
        send_message(chat_id, f"✅ Часть сохранена! ({len(history)} частей)\n📊 Всего символов: {len(new_code)}\n\n📝 Показать код - посмотреть всё\n🗑 Удалить последний - отменить")
