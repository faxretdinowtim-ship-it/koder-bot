...
# ==================== ФУНКЦИЯ ГЕНЕРАЦИИ HTML СТРАНИЦЫ С КОДОМ ====================
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .info {{ display: flex; gap: 20px; margin-top: 15px; }}
...
<div class="container">
    ...
    <div class="footer">🤖 Создано с помощью AI Code Bot</div>
</div>
<script>
    function copyCode() {{
...
# ==================== ФУНКЦИЯ СОЗДАНИЯ ФАЙЛА С ПОДПИСЬЮ BOT ====================
def save_code_file(user_id, filename, content):
    """Сохраняет код в файл с подписью _bot"""
    base_name, ext = os.path.splitext(filename)
    bot_filename = f"{base_name}_bot{ext}"
    
    # Добавляем подпись о боте в начало файла
...
<style>body{{background:#1e1e1e;}} #editor{{height:80vh;}}</style>
...
def process_message(msg):
    ...
    # НОВАЯ КНОПКА: ИСТОРИЯ КОДА
    elif text == "📜 История кода" or text == "/history":
        history = user_sessions[uid].get("history", [])
        if not history:
            send_message(chat_id, "📭 История пуста. Отправьте код, чтобы сохранить его в историю!")
        else:
            history_text = "📜 *История кода:*\n\n"
            for i, entry in enumerate(history[-10:], 1):
                date = entry.get('time', '')[:16]
                preview = entry.get('preview', '')[:50]
                history_text += f"{i}. [{date}] `{preview}...`\n"
            history_text += f"\n📊 Всего записей: {len(history)}\n💡 /load_history {len(history)} — восстановить версию"
            send_message(chat_id, history_text, parse_mode="Markdown")
    
    elif text == "/load_history":
        send_message(chat_id, "📝 Введите номер версии для восстановления (например: /load 3)")
        ...
        try:
            ...
            if 1 <= version <= len(history):
                entry = history[version - 1]
                if 'code' in entry:
                    user_sessions[uid]["files"][user_sessions[uid].get("current_file", "main.py")] = entry['code']
                    send_message(chat_id, f"✅ Восстановлена версия {version} от {entry.get('time', '')[:16]}")
                else:
                    send_message(chat_id, "❌ Не удалось восстановить код")
            else:
                send_message(chat_id, f"❌ Версия {version} не найдена. Всего версий: {len(history)}")
        except:
            send_message(chat_id, "❌ Используйте: /load <номер>")
    
    elif text == "/sandbox" or text == "🐍 Песочница":
        send_message(chat_id, f"🐍 *Отдельная песочница для кода:*\n{bot_url}/sandbox", parse_mode="Markdown")
    ...
    elif text == "💾 Скачать код (bot)" or text == "/done":
        files = user_sessions[uid].get("files", {})
        current = user_sessions[uid].get("current_file", "main.py")
        code = files.get(current, "")
        if not code.strip():
            ...
            return
        bot_filename = save_code_file(uid, current, code)
        with open(bot_filename, "rb") as f:
            requests.post(f"{API_URL}/sendDocument", data={"chat_id": chat_id, "caption": f"✅ {current} (с подписью _bot)"}, files={"document": f})
        ...
        send_message(chat_id,
            "📚 *Команды:*\n\n"
            "📜 /history — история кода (сохраняет каждую версию)\n"
            "🌍 /publish — опубликовать как сайт\n"
            "🐍 /sandbox — отдельная песочница\n"
            "💾 /done — скачать с подписью _bot\n"
            "🌐 /web — веб-редактор\n"
            "🔧 /fix — исправить ошибки\n"
            "🏃 /run — выполнить код\n"
            "📁 /files — список файлов",
        ...
        send_message(chat_id, f"✅ Код добавлен в `{current}`! {len(new_code)} символов\n📜 /history — посмотреть историю", parse_mode="Markdown")
